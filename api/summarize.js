...
// This is a Vercel Edge Function, which runs in a secure environment closer to users for faster performance.
...
export default async function handler(request) {
    ...
    try {
        ...
        const { aboutText1, aboutText2, skills, experience, projects } = body;

        // Basic validation to ensure we have some data
        if (!aboutText1 || !skills || !experience) {
            return new Response(JSON.stringify({ error: 'Missing required profile data.' }), {
                status: 400,
                headers: { 'Content-Type': 'application/json' }
            });
        }
        ...
        // 5. Call the Google Gemini API
        const apiUrl = `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${apiKey}`;  // Switched to stable 'gemini-1.5-flash' (adjust if using a different version)
